...
static esp_err_t set_window(st7789_display_t* display, uint16_t x, uint16_t y, uint16_t width, uint16_t height) {
    ...
    ESP_RETURN_ON_ERROR(write_command_data(display, ST7789_CMD_CASET, column_data, sizeof(column_data)), TAG,
                        "set columns failed");
    ESP_RETURN_ON_ERROR(write_command_data(display, ST7789_CMD_RASET, row_data, sizeof(row_data)), TAG,
                        "set rows failed");
...
static bool rect_is_valid(const st7789_display_t* display, uint16_t x, uint16_t y, uint16_t width, uint16_t height) {
    return width > 0 && height > 0 && x < display->board->display_width && y < display->board->display_height &&
           width <= display->board->display_width - x && height <= display->board->display_height - y;
}
...
static esp_err_t init_panel(st7789_display_t* display) {
    ...
    vTaskDelay(pdMS_TO_TICKS(120));
    ESP_RETURN_ON_ERROR(write_command_data(display, ST7789_CMD_COLMOD, &color_mode, sizeof(color_mode)), TAG,
                        "color mode failed");
    vTaskDelay(pdMS_TO_TICKS(10));
    ESP_RETURN_ON_ERROR(write_command_data(display, ST7789_CMD_MADCTL, &madctl, sizeof(madctl)), TAG,
                        "orientation failed");
...
esp_err_t st7789_display_init(st7789_display_t** display_out) {
    ...
    *display_out = NULL;
    ...
    ESP_GOTO_ON_ERROR(spi_bus_initialize(board->spi_host, &bus_config, SPI_DMA_CH_AUTO), cleanup, TAG,
                      "spi bus init failed");
    ...
    ESP_GOTO_ON_ERROR(spi_bus_add_device(board->spi_host, &device_config, &display->spi), cleanup_bus, TAG,
    ...
    ESP_LOGI(TAG, "Initialized ST7789 display %ux%u", board->display_width, board->display_height);
    *display_out = display;
    return ESP_OK;
...
esp_err_t st7789_display_push_pixels(st7789_display_t* display, uint16_t x, uint16_t y, uint16_t width, uint16_t height,
    ...
    while (remaining > 0) {
        ...
        ESP_RETURN_ON_ERROR(write_data(display, (const uint8_t*)display->scratch, chunk_pixels * sizeof(uint16_t)), TAG,
                            "pixel push failed");
        cursor += chunk_pixels;
...
esp_err_t st7789_display_fill(st7789_display_t* display, uint16_t color565) {
    ESP_RETURN_ON_FALSE(display != NULL, ESP_ERR_INVALID_ARG, TAG, "display is required");
    return st7789_display_draw_rect(display, 0, 0, display->board->display_width, display->board->display_height,
                                    color565);
}
