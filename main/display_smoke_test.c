...
static esp_err_t draw_border(st7789_display_t* display, uint16_t width, uint16_t height, uint16_t color,
                             uint16_t thickness) {
    ESP_RETURN_ON_ERROR(st7789_display_draw_rect(display, 0, 0, width, thickness, color), TAG, "top border failed");
    ESP_RETURN_ON_ERROR(st7789_display_draw_rect(display, 0, height - thickness, width, thickness, color), TAG,
                        "bottom border failed");
...
static esp_err_t draw_checker(st7789_display_t* display, uint16_t x, uint16_t y) {
    ...
    return st7789_display_push_pixels(display, x, y, CHECKER_WIDTH, CHECKER_HEIGHT, checker_pixels,
...
static esp_err_t draw_diagonal_pixels(st7789_display_t* display, uint16_t width, uint16_t height) {
    ...
    for (uint16_t i = 0; i < limit; ++i) {
        ...
        ESP_RETURN_ON_ERROR(st7789_display_draw_pixel(display, width - i - 1, i, COLOR_CYAN), TAG,
                            "secondary diagonal failed");
    }
...
static esp_err_t draw_block_o(st7789_display_t* display, uint16_t x, uint16_t y, uint16_t scale, uint16_t color) {
    ...
    ESP_RETURN_ON_ERROR(st7789_display_draw_rect(display, x, y + (6 * scale), 5 * scale, scale, color), TAG,
                        "O bottom failed");
...
static esp_err_t draw_block_k(st7789_display_t* display, uint16_t x, uint16_t y, uint16_t scale, uint16_t color) {
    ...
    for (uint16_t step = 0; step < 4; ++step) {
        ESP_RETURN_ON_ERROR(
            st7789_display_draw_rect(display, x + ((1 + step) * scale), y + ((3 - step) * scale), scale, scale, color),
            TAG, "K upper failed");
        ESP_RETURN_ON_ERROR(
            st7789_display_draw_rect(display, x + ((1 + step) * scale), y + ((3 + step) * scale), scale, scale, color),
            TAG, "K lower failed");
