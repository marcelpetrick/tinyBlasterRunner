...
static void save_load(save_data_t* save) {
    ...
    nvs_close(h);
    ESP_LOGI(TAG, "Save loaded: stars=%lu taken=%lu found=%lu", (unsigned long)save->stars,
             (unsigned long)save->photos_taken, (unsigned long)save->species_found);
}
...
static esp_err_t draw_background(monster_game_t* game) {
    ...
    ESP_RETURN_ON_ERROR(graphics_fill_rect(g, 0, ZONE_SKY_END, DISP_W, ZONE_GRASS_END - ZONE_SKY_END, C_GRASS), TAG,
                        "bg");
    ESP_RETURN_ON_ERROR(graphics_fill_rect(g, 0, ZONE_GRASS_END, DISP_W, ZONE_HUD_START - ZONE_GRASS_END, C_GROUND),
                        TAG, "bg");
...
static int32_t monster_screen_x(const monster_game_t* game, int idx) {
    return MONSTER_INFO[idx].spawn_x + (game->monsters[idx].world_x - MONSTER_INFO[idx].spawn_x) - game->camera_x;
}
...
static esp_err_t clear_sprite_area(graphics_t* g, int32_t sx, int32_t sy, uint16_t w, uint16_t h) {
    ...
    for (uint16_t row = 0; row < h; row++) {
        ...
        ESP_RETURN_ON_ERROR(graphics_fill_rect(g, (uint16_t)x0, (uint16_t)py, (uint16_t)(x1 - x0), 1, bg), TAG,
                            "clear sprite");
    }
    return ESP_OK;
}
...
static esp_err_t draw_state_indicator(monster_game_t* game, int idx, int32_t sx, int32_t sy) {
    ...
    if (state != MSTATE_SLEEPING && state != MSTATE_PLAYING)
        return ESP_OK;

    int32_t ix = sx + (int32_t)(MONSTER_INFO[idx].w * SPRITE_SCALE) / 2 - 3;
    int32_t iy = sy - 10;
    if (iy < 0 || ix < 0 || ix >= DISP_W)
        return ESP_OK;

    uint16_t clr = (state == MSTATE_SLEEPING) ? C_WHITE : C_YELLOW;
    const char* label = (state == MSTATE_SLEEPING) ? "Z" : "!";
    // clear background first
    ESP_RETURN_ON_ERROR(graphics_fill_rect(g, (uint16_t)ix, (uint16_t)iy, 6, 8, bg_color_at_y(iy)), TAG, "ind clear");
...
static esp_err_t draw_monster(monster_game_t* game, int idx) {
    ...
    ESP_RETURN_ON_ERROR(graphics_draw_sprite_scaled(g, sx, sy, info->w, info->h, info->sprite, SPRITE_KEY, DISP_W,
                                                    DISP_H, SPRITE_SCALE),
...
static esp_err_t draw_camera_overlay(monster_game_t* game) {
    ...
    // label
    ESP_RETURN_ON_ERROR(graphics_draw_text(g, (uint16_t)(DISP_W / 2 - 15), 24, "KAMERA", C_WHITE, 1), TAG, "cam label");
    return ESP_OK;
}
...
static esp_err_t draw_discovery(monster_game_t* game) {
    ...
    if (game->needs_full_redraw) {
        ...
        uint32_t stars_earned = (game->monsters[game->photo_species].rarity == 0)   ? 10u
                                : (game->monsters[game->photo_species].rarity == 1) ? 25u
                                                                                    : 50u;
        ...
        int32_t ssy = 165;
        ESP_RETURN_ON_ERROR(graphics_draw_sprite_scaled(g, ssx, ssy, info->w, info->h, info->sprite, SPRITE_KEY, DISP_W,
                                                        DISP_H, SPRITE_SCALE),
                            TAG, "disc sprite");

        ESP_RETURN_ON_ERROR(graphics_fill_rect(g, 0, ZONE_HUD_START, DISP_W, DISP_H - ZONE_HUD_START, C_HUD_BG), TAG,
                            "disc hud");
        ESP_RETURN_ON_ERROR(graphics_draw_text(g, 20, ZONE_HUD_START + 1, "TASTE DRUECKEN", C_TEXT, 1), TAG,
                            "disc prompt");
    }
    return ESP_OK;
}
...
static esp_err_t draw_result(monster_game_t* game) {
    ...
    if (game->photo_species >= 0 && game->photo_species < MONSTER_SPECIES_COUNT) {
        ...
        // Draw monster sprite
        int32_t ssx = (DISP_W - (int32_t)(info->w * SPRITE_SCALE)) / 2;
        ESP_RETURN_ON_ERROR(graphics_draw_sprite_scaled(g, ssx, 130, info->w, info->h, info->sprite, SPRITE_KEY, DISP_W,
                                                        DISP_H, SPRITE_SCALE),
                            TAG, "res sprite");
    }
    ...
    ESP_RETURN_ON_ERROR(graphics_fill_rect(g, 0, ZONE_HUD_START, DISP_W, DISP_H - ZONE_HUD_START, C_HUD_BG), TAG,
                        "res hud");
    ESP_RETURN_ON_ERROR(graphics_draw_text(g, 20, ZONE_HUD_START + 1, "TASTE DRUECKEN", C_TEXT, 1), TAG, "res prompt");
...
static esp_err_t draw_book(monster_game_t* game) {
    ...
    ESP_RETURN_ON_ERROR(graphics_draw_text(g, 4, 24, "STERNE GESAMT", C_YELLOW, 1), TAG, "book st lbl");
    ...
    for (int i = 0; i < MONSTER_SPECIES_COUNT; i++) {
        ...
        if (game->save.discovered[i]) {
            ...
            const char* rar = (MONSTER_INFO[i].rarity == 0)   ? "HAEUFIG"
                              : (MONSTER_INFO[i].rarity == 1) ? "SELTEN"
                                                              : "LEGENDAER";
            uint16_t rc = (MONSTER_INFO[i].rarity == 0) ? C_TEXT : (MONSTER_INFO[i].rarity == 1) ? C_ORANGE : C_RED;
            ...
            ESP_RETURN_ON_ERROR(graphics_draw_sprite_scaled(g, ssx, ssy, info->w, info->h, info->sprite, SPRITE_KEY,
    ...
    ESP_RETURN_ON_ERROR(graphics_fill_rect(g, 0, ZONE_HUD_START, DISP_W, DISP_H - ZONE_HUD_START, C_HUD_BG), TAG,
                        "book hud");
    ESP_RETURN_ON_ERROR(graphics_draw_text(g, 2, ZONE_HUD_START + 1, "L:WEITER L-LANG:SCHLIES", C_TEXT, 1), TAG,
                        "book hud t");
...
static esp_err_t render_explore(monster_game_t* game) {
    ...
    if (game->needs_full_redraw) {
        ...
        ESP_RETURN_ON_ERROR(graphics_draw_sprite_scaled(g, psx, PLAYER_SCREEN_Y, PLAYER_SPRITE_W, PLAYER_SPRITE_H,
    ...
    if (new_psx != game->prev_player_sx) {
        ESP_RETURN_ON_ERROR(clear_sprite_area(g, game->prev_player_sx, PLAYER_SCREEN_Y, PLAYER_SPRITE_W * SPRITE_SCALE,
                                              PLAYER_SPRITE_H * SPRITE_SCALE),
                            TAG, "clear player");
        ESP_RETURN_ON_ERROR(graphics_draw_sprite_scaled(g, new_psx, PLAYER_SCREEN_Y, PLAYER_SPRITE_W, PLAYER_SPRITE_H,
                                                        PLAYER_SPRITE, SPRITE_KEY, DISP_W, DISP_H, SPRITE_SCALE),
                            TAG, "draw player");
    ...
    for (int i = 0; i < MONSTER_SPECIES_COUNT; i++) {
        ...
        ESP_RETURN_ON_ERROR(clear_sprite_area(g, old_sx, clr_y, (uint16_t)(info->w * SPRITE_SCALE), clr_h), TAG,
                            "clear m");
...
esp_err_t monster_game_handle_input(monster_game_t* game, const button_event_t* events, size_t event_count) {
                    ...
                    if (sidx >= 0) {
                        ...
                        if (!game->save.discovered[sidx]) {
                            ...
                            uint32_t bonus = (MONSTER_INFO[sidx].rarity == 0)   ? 10u
                                             : (MONSTER_INFO[sidx].rarity == 1) ? 25u
                                                                                : 50u;
                            game->save.stars += bonus;
                        } else {
                            game->photo_new = false;
                            if (game->photo_score >= 50)
                                game->save.stars += 5u;
                        }
                    }
