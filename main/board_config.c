...
void board_config_log(void) {
    ...
    ESP_LOGI(TAG, "Display: %s %ux%u offset=%u,%u", BOARD_CONFIG.display_driver, BOARD_CONFIG.display_width,
             BOARD_CONFIG.display_height, BOARD_CONFIG.display_x_offset, BOARD_CONFIG.display_y_offset);
    ESP_LOGI(TAG, "TFT SPI: host=%d clock=%luHz MOSI=%d SCLK=%d CS=%d DC=%d RST=%d BL=%d", BOARD_CONFIG.spi_host,
             (unsigned long)BOARD_CONFIG.spi_clock_hz, BOARD_CONFIG.tft_mosi, BOARD_CONFIG.tft_sclk,
             BOARD_CONFIG.tft_cs, BOARD_CONFIG.tft_dc, BOARD_CONFIG.tft_rst, BOARD_CONFIG.tft_bl);
    ESP_LOGI(TAG, "Buttons: left GPIO%d, right GPIO%d, active level=%d", BOARD_CONFIG.button_left,
             BOARD_CONFIG.button_right, BOARD_CONFIG.button_active_level);
    ESP_LOGI(TAG, "Battery ADC: GPIO%d, divider ratio=%.2f", BOARD_CONFIG.battery_adc,
             BOARD_CONFIG.battery_divider_ratio);
    ESP_LOGI(TAG, "Expected flash: %lu bytes, serial baud: %lu", (unsigned long)BOARD_CONFIG.expected_flash_size,
