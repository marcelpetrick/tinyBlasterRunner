...
static esp_err_t draw_score(demo_game_t* game) {
    ESP_RETURN_ON_ERROR(graphics_fill_rect(game->graphics, 1, 1, 96, HUD_HEIGHT - 2, COLOR_BACKGROUND), "demo_game",
                        "score clear failed");
    ESP_RETURN_ON_ERROR(graphics_draw_text(game->graphics, 4, 5, "SCORE", COLOR_TEXT, 1), "demo_game",
                        "score label failed");
...
esp_err_t demo_game_render(demo_game_t* game) {
    ...
    if (game->previous_item_x >= 0) {
        ESP_RETURN_ON_ERROR(graphics_fill_rect(game->graphics, (uint16_t)game->previous_item_x,
                                               (uint16_t)game->previous_item_y, ITEM_SIZE, ITEM_SIZE, COLOR_BACKGROUND),
                            "demo_game", "old item clear failed");
    }
    ESP_RETURN_ON_ERROR(graphics_fill_rect(game->graphics, (uint16_t)game->item_x, (uint16_t)item_y, ITEM_SIZE,
                                           ITEM_SIZE, game->score == 0 ? COLOR_MISS : COLOR_ITEM),
                        "demo_game", "item render failed");

    if (game->redraw_all || game->previous_paddle_x != game->paddle_x) {
        if (game->previous_paddle_x >= 0) {
            ESP_RETURN_ON_ERROR(graphics_fill_rect(game->graphics, (uint16_t)game->previous_paddle_x,
                                                   (uint16_t)paddle_y, PADDLE_WIDTH, PADDLE_HEIGHT, COLOR_BACKGROUND),
                                "demo_game", "old paddle clear failed");
        }
        ESP_RETURN_ON_ERROR(graphics_fill_rect(game->graphics, (uint16_t)game->paddle_x, (uint16_t)paddle_y,
                                               PADDLE_WIDTH, PADDLE_HEIGHT, COLOR_PADDLE),
                            "demo_game", "paddle render failed");
    }
