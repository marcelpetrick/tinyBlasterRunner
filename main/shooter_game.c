...
static void sfb_sprite(int x, int y, int sw, int sh, const uint16_t* pix, uint8_t scale) {
        ...
        for (int s = 0; s < scale; s++) {
            ...
            for (int col = 0; col < sw; col++) {
                ...
                for (int ss = 0; ss < scale; ss++) {
                    ...
                    if ((unsigned)px < (unsigned)DISP_W)
                        dst[px] = c;
                }
            }
        }
...
static void sfb_outline(int x, int y, int w, int h, uint16_t c) {
    sfb_fill(x, y, w, 1, c);
    if (h > 1)
        sfb_fill(x, y + h - 1, w, 1, c);
    if (h > 2) {
        sfb_fill(x, y + 1, 1, h - 2, c);
        if (w > 1)
            sfb_fill(x + w - 1, y + 1, 1, h - 2, c);
    }
}
...
static void nvs_load_hi(int32_t* hi) {
    *hi = 0;
    nvs_handle_t h;
    if (nvs_open("tbr", NVS_READONLY, &h) != ESP_OK)
        return;
    uint32_t v = 0;
    nvs_get_u32(h, "hi", &v);
    *hi = (int32_t)v;
...
static void nvs_save_hi(int32_t hi) {
    nvs_handle_t h;
    if (nvs_open("tbr", NVS_READWRITE, &h) != ESP_OK)
        return;
...
static void try_spawn_coin(shooter_game_t* g) {
    ...
    switch (pattern) {
        case 0:
            y_pos = (float)(GROUND_Y - COIN_DRAW_H - 2);
            break;
        case 1:
            y_pos = (float)(PLAYER_GND_Y - 22);
            break;
        default:
            y_pos = (float)(PLAYER_GND_Y - 44);
            break;
    }
    int spawned = 0;
...
static void try_spawn_enemy(shooter_game_t* g) {
    ...
    sg_enemy_type_t type = (roll == 0) ? SG_ENEMY_GROUND_BOT : (roll == 1) ? SG_ENEMY_FLYING_DRONE : SG_ENEMY_SLUG;
    float y;
...
static void try_spawn_obstacle(shooter_game_t* g) {
    ...
    float y = (type == SG_OBS_ROCK) ? (float)(GROUND_Y - ROCK_DRAW_H) : (float)(GROUND_Y - CRATE_DRAW_H);
        ...
        if (game->state == SG_STATE_PLAY) {
            ...
            if (ev->type == BUTTON_EVENT_PRESSED && ev->id == BUTTON_ID_LEFT) {
                if (!p->in_air) {
                    ...
                    p->left_held = true;
                    p->left_hold_ms = 0;
                    spawn_sparkle(game, PLAYER_FIXED_X + ROBOT_DRAW_W / 2.0f, p->y + ROBOT_DRAW_H / 2.0f);
                }
            }
...
esp_err_t shooter_game_update(shooter_game_t* game, uint32_t frame, uint32_t dt_ms) {
    ...
    // Shoot cooldown + anim
    if (p->shoot_cooldown_ms > dt_ms)
        p->shoot_cooldown_ms -= dt_ms;
    else
        p->shoot_cooldown_ms = 0;
    ...
    for (int i = 0; i < SG_MAX_PROJECTILES; i++) {
        ...
        if (game->projectiles[i].x > (float)DISP_W)
    ...
    for (int i = 0; i < SG_MAX_OBSTACLES; i++) {
        ...
        if (o->x + (float)CRATE_DRAW_W < 0.0f)
            o->active = false;
    }
    ...
    for (int i = 0; i < SG_MAX_PARTICLES; i++) {
        ...
        pt->x += pt->vx;
        pt->y += pt->vy;
        if (pt->type != SG_PART_SPARKLE && pt->type != SG_PART_SCORE_TEXT)
            pt->vy += 0.1f;
        if (pt->life_ms > dt_ms)
            pt->life_ms -= dt_ms;
        else
    ...
    for (int pi = 0; pi < SG_MAX_PROJECTILES; pi++) {
        ...
        for (int ei = 0; ei < SG_MAX_ENEMIES; ei++) {
            ...
            float ew = (e->type == SG_ENEMY_SLUG)           ? (float)SLUG_DRAW_W
                       : (e->type == SG_ENEMY_FLYING_DRONE) ? (float)DRONE_DRAW_W
                                                            : (float)EBOT_DRAW_W;
            float eh = (e->type == SG_ENEMY_SLUG)           ? (float)SLUG_DRAW_H
                       : (e->type == SG_ENEMY_FLYING_DRONE) ? (float)DRONE_DRAW_H
                                                            : (float)EBOT_DRAW_H;
            if (aabb(game->projectiles[pi].x, game->projectiles[pi].y, PROJ_W, PROJ_H, e->x, e->y, ew, eh)) {
    ...
    for (int pi = 0; pi < SG_MAX_PROJECTILES; pi++) {
        ...
        for (int oi = 0; oi < SG_MAX_OBSTACLES; oi++) {
            ...
            if (aabb(game->projectiles[pi].x, game->projectiles[pi].y, PROJ_W, PROJ_H, o->x, o->y, ow, oh)) {
                game->projectiles[pi].active = false;
    ...
    for (int ei = 0; ei < SG_MAX_ENEMIES; ei++) {
        ...
        float eh = (e->type == SG_ENEMY_SLUG)           ? (float)SLUG_DRAW_H
                   : (e->type == SG_ENEMY_FLYING_DRONE) ? (float)DRONE_DRAW_H
                                                        : (float)EBOT_DRAW_H;
        if (aabb(PLAYER_FIXED_X + 4, p->y + 4, (float)ROBOT_DRAW_W - 8, (float)ROBOT_DRAW_H - 8, e->x + 2, e->y + 2,
                 ew - 4, eh - 4)) {
            e->active = false;
    ...
    for (int oi = 0; oi < SG_MAX_OBSTACLES; oi++) {
        ...
        float ow = (o->type == SG_OBS_ROCK) ? (float)ROCK_DRAW_W : (float)CRATE_DRAW_W;
        float oh = (o->type == SG_OBS_ROCK) ? (float)ROCK_DRAW_H : (float)CRATE_DRAW_H;
        if (aabb(PLAYER_FIXED_X + 4, p->y + 4, (float)ROBOT_DRAW_W - 8, (float)ROBOT_DRAW_H - 8, o->x + 2, o->y, ow - 4,
                 oh)) {
            o->active = false;
            player_take_damage(game);
        }
    ...
    for (int ci = 0; ci < SG_MAX_COINS; ci++) {
        ...
        if (!c->active)
            continue;
        if (aabb(PLAYER_FIXED_X + 2, p->y + 2, (float)ROBOT_DRAW_W - 4, (float)ROBOT_DRAW_H - 4, c->x, c->y,
                 (float)COIN_DRAW_W, (float)COIN_DRAW_H)) {
            c->active = false;
    ...
    // Spawn logic
    {
        ...
        if (game->game_ms - game->last_obstacle_ms >= o_int) {
            if ((rng_next(&game->rng) % 10u) < 7u)
                try_spawn_obstacle(game);
...
static void draw_gameplay(shooter_game_t* g) {
    ...
    for (int i = 0; i < SG_MAX_PARTICLES; i++) {
        ...
        if (!pt->active || pt->type == SG_PART_SCORE_TEXT)
            continue;
        int px = (int)pt->x, py = (int)pt->y;
        if (px < 0 || px >= DISP_W || py < HUD_H || py >= DISP_H)
            continue;
    ...
    // Player
    if (g->player.visible) {
        const uint16_t* spr = g->player.in_air ? ROBOT_JUMP : (g->player.run_frame == 0 ? ROBOT_RUN0 : ROBOT_RUN1);
        sfb_sprite(PLAYER_FIXED_X, (int)g->player.y, ROBOT_SPR_W, ROBOT_SPR_H, spr, SPR_SCALE);
    }
    ...
    for (int i = 0; i < SG_MAX_PROJECTILES; i++) {
        ...
        if (px < 0 || px + PROJ_W > DISP_W)
            continue;
        if (py < HUD_H || py + PROJ_H > DISP_H)
            continue;
        sfb_fill(px, py, PROJ_W, PROJ_H, C_CYAN);
        if (px > 3)
            sfb_fill(px - 3, py + 1, 3, PROJ_H - 2, C_PROJ_TRAIL);
    }
    ...
    for (int i = 0; i < SG_MAX_PARTICLES; i++) {
        ...
        if (!pt->active || pt->type != SG_PART_SCORE_TEXT)
            continue;
        int px = (int)pt->x, py = (int)pt->y;
        if (px < 0 || px > DISP_W - 20 || py < HUD_H || py >= DISP_H)
            continue;
        char buf[8];
...
static void draw_splash(shooter_game_t* g) {
    ...
    sfb_sprite((DISP_W - ROBOT_DRAW_W) / 2, 135, ROBOT_SPR_W, ROBOT_SPR_H, ROBOT_RUN0, SPR_SCALE);
...
static void draw_menu(shooter_game_t* g) {
    ...
    sfb_sprite((DISP_W - ROBOT_DRAW_W) / 2, 98, ROBOT_SPR_W, ROBOT_SPR_H, ROBOT_RUN1, SPR_SCALE);
...
esp_err_t shooter_game_render(shooter_game_t* game) {
    switch (game->state) {
        ...
        case SG_STATE_PLAY:
            draw_gameplay(game);
            break;
        case SG_STATE_GAMEOVER:
            draw_gameover(game);
            break;
    }
    return graphics_draw_bitmap(game->graphics, 0, 0, DISP_W, DISP_H, s_fb, (size_t)DISP_W * DISP_H);
}
